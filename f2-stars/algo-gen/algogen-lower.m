...
(* ::Text:: *)
(*For lower bounds, we consider all facility groups. We will restrict ourselves to the case when |F2|>=2*|F1|, meaning the C-groups can always be as large as their matching A-groups.*)
...
initialize[t_]:=(
	n = t;
	gammas = Table[createVar["gamma",i],{i,0,n}];
	pAs = Table[createVar["pA",i],{i,1,n}];
	pBs = Table[createVar["pB",i],{i,1,n}];
	pCs = Table[createVar["pC",i],{i,0,n}];
	varp = Union[pAs,pBs,pCs];
	cond = Union[{0<b<1},0<=#<=1&/@gammas,0<=#<=1&/@varp];
	massExpr= Total[MapThread[(#1+#2+#3)#4&,{pAs,pBs,pCs[[2;;]],gammas[[2;;]]}]]+gamma0*pC0==1+b+b*gamma0;
);
